...
export interface WalineProps {
   ...
   *
   * 设置为 `0` 时无限制 Comment word s limit. When a single number is filled in, it 's the maximum number
   * of comment words.
   *
   ...
   * 是否启用暗黑模式适配
   *
   * 设置 `'auto'` 会根据设备暗黑模式自适应。填入 CSS 选择器会在对应选择器生效时启用夜间模式。 Whether to enable darkmode support
   *
