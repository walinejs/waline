...
- `SMTP_SERVICE`: SMTP Mail-Zustelldienst-Anbieter
  ...
  Sie können alle unterstützten Anbieter in [nodemailer services](https://github.com/nodemailer/nodemailer/blob/master/src/well-known/services.json) finden. Wenn Ihr Anbieter nicht aufgeführt ist, müssen Sie `SMTP_HOST` und `SMTP_PORT` konfigurieren.
