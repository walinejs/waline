...
Unterstützte Serviceanbieter finden Sie unter [nodemailer services](https://github.com/nodemailer/nodemailer/blob/master/src/well-known/services.json). Sie können zwischen `SMTP_SERVICE` und (`SMTP_HOST`, `SMTP_PORT`) wählen. Wenn Sie den entsprechenden `SMTP_SERVICE` in der Liste nicht kennen, müssen Sie `SMTP_HOST` und `SMTP_PORT` konfigurieren, die normalerweise in den Postfacheinstellungen zu finden sind.
