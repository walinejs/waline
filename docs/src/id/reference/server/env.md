...
Penyedia layanan yang didukung dapat ditemukan di [layanan nodemailer](https://github.com/nodemailer/nodemailer/blob/master/src/well-known/services.json). Anda dapat memilih salah satu dari `SMTP_SERVICE` dan (`SMTP_HOST`, `SMTP_PORT`). Jika Anda tidak mengetahui `SMTP_SERVICE` yang sesuai dalam daftar, Anda perlu mengonfigurasi `SMTP_HOST` dan `SMTP_PORT`, yang mungkin dapat ditemukan di pengaturan kotak surat.
