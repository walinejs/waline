...
- `SMTP_SERVICE`: SMTP 메일 전송 서비스 제공자
  ...
  [nodemailer services](https://github.com/nodemailer/nodemailer/blob/master/src/well-known/services.json)에서 지원하는 모든 제공자를 확인할 수 있습니다. 목록에 없는 제공자를 사용하는 경우 `SMTP_HOST`와 `SMTP_PORT`를 설정해야 합니다.
