...
1. 按照 [Vercel 部署](./quick-start.md#vercel-部署) 一节部署服务端。`LEAN_ID` 和 `LEAN_KEY` 和之前使用 Valine 申请的保持一致即可。同时请不要忘记为 Waline 设置 `LEAN_MASTER_KEY`。
2. 按照 [HTML 片段](./quick-start.md#html-片段) 一节修改对应的前端脚本。
