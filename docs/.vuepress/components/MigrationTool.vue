...
function commento2lc(input) {
  ...
  const hexMaps = {};
  comments.filter(comment => comment.parentHex && comment.parentHex !== 'root').forEach(comment => {
  ...
  return {
    results: comments.filter(comment => !comment.deleted).map(comment => {
      const commenter = commenters[comment.commenterHex] ? commenters[comment.commenterHex] : {
        nick: 'Anonymous',
        mail: '',
        link: ''
      };
      ...
      return Object.assign({
        ...
        rid: rootHexMaps[comment.commentHex] ? rootHexMaps[comment.commentHex] : '',
